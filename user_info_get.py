...
def main():
    ...
    def user_info(file_name, start, end=None):
        ...
        try:
                ...
                for row_index in range(1, len(col_values_nonnull)):     #行循环
                    ...
                    try:
                        ...
                        if len(error) > 100:    #如果错误过多（估计被封ip），则退出
                            error.append('\n终止在{}列，已采集{:^5}个数据，错误率：{:^3}%，采集率{:^3}%'.format(col_index, info_count, round(100/info_count, 2), round(info_count/info_max, 2)))
                            print('\n终止在{}列，已采集{:^5}个数据，错误率：{:^3}%，采集率{:^3}%'.format(col_index, info_count, round(100/info_count, 2), round(info_count/info_max, 2)))
                            raise
            ...
            print('\n解析完成！已采集{:^5}个数据，错误率：{:^3}%，采集率{:^3}%'.format(info_count, round(len(error)/info_count, 2), round(info_count/info_max, 2)))
        except:
            print("\n因其他错误退出！")
